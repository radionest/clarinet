...
`on_status("pending")` срабатывает и при hard-инвалидации записи, которая уже была в `pending` — повторная инвалидация перезапускает флоу. Обработчики (`.do_task`, `.call`) обязаны быть идемпотентными.
