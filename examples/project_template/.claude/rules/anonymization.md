...
> That surfaces two ways: at **worker** startup once `have_dicom` is on, and in the
> **API** process (`pipeline_enabled=true`, even with `have_dicom=false`) as an
> HTTP 500 from the anonymize endpoint's deferred import. Pick a distinct name
