"""Slicer script — second review: classify missed defects.

Subtracts the inspector's segmentation from the master projection to find
missed ROIs, then presents a classification segmentation for the reviewer.
...
Context variables (injected by build_slicer_context):
    ...
    segmentation: Path to the inspector's initial segmentation (auto, from file_registry).
...
# Load projection and inspector segmentation
projection = s.load_segmentation(master_projection, "Projection")  # type: ignore[name-defined]  # noqa: F821
inspector_seg = s.load_segmentation(segmentation, "InspectorSeg")  # type: ignore[name-defined]  # noqa: F821

# Subtract inspector's segmentation from projection to find missed defects
missed = s.subtract_segmentations(projection, inspector_seg, "MissedDefects")
...
# Merge missed defects into Classification as _pool for cross-segmentation Islands
s.merge_as_pool(missed, classification)

# Hide auxiliary segmentations, keep Classification and InspectorSeg visible
...
s.annotate("Classify missed defects")
