...
async def validate_record_files(
    ...
    *,
    ...
    # any anonymization run). `lenient_working_dir` falls back to raw UIDs
    # so a record that has not been anonymized yet still gets a
    # `valid/invalid` verdict instead of bubbling `AnonPathError` up to
    # the caller. The full `working_dirs` map is still needed for
    # cross-level file-def lookups inside `FileValidator.validate`.
    directory = FileResolver.lenient_working_dir(record)
