...
class RecordService:
    ...
    async def check_files(self, record_id: int) -> tuple[list[str], dict[str, str]]:
        ...
        # `lenient_working_dir` falls back to raw UIDs so a record whose
        # study/series has not been anonymized yet still gets a checksum
        # verdict instead of raising AnonPathError (matches the legacy
        # `record.working_folder` behaviour, see file-repo roadmap §4 —
        # strict-only repository tightening is deferred to its own PR).
        new_checksums = await compute_checksums(
            record_read.record_type.file_registry or [],
            record_read,
            FileResolver.lenient_working_dir(record_read),
        )
        old_checksums = {
    ...
    async def _resolve_paths_for_file_def(
        ...
        """
        # Admin/UI-triggered cascade — `lenient_working_dir` falls back to
        # raw UIDs for records whose study/series has not been anonymized
        # yet, otherwise the delete pipeline would 500 on legacy data. The
        # full `working_dirs` map is still needed for cross-level
        # ``file_def.level`` lookups.
        default_dir = FileResolver.lenient_working_dir(record_read)
