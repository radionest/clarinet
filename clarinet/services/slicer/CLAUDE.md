...
Key methods: `SlicerHelper(working_folder)`, `load_volume()`, `create_segmentation()`, `load_segmentation()`, `setup_editor()`, `detect_overlaps()`, `subtract_segmentations()`, `load_study_from_pacs()`, `load_series_from_pacs()`, `set_dual_layout()`, `align_by_center()`, `setup_segment_focus_observer()`.
...
- `POST /exec` — execute script with helper DSL prepended; optional `include_correspondence: bool` requests the correspondence-engine bundle; `/records/{record_id}/open`, `/records/{record_id}/validate`, and the submit-path validator always include it (D9, slicer-engine-parity)
