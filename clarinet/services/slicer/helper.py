...
def _reindex_segmentation_to_grid(source_node: Any, ref_grid: Grid) -> tuple[Any, Any]:
    ...
    *source_node* is only ever read -- never mutated.
    ...
    segments keep their source id unless it collides with an id already
    assigned to a baked segment, in which case Slicer hands back a different
    unique id instead (the actually-assigned id, not the requested one, is
    what gets used to restore the label value); a voxeled segment's id may
    also be regenerated by Slicer on import -- ``RemoveSegment``+``AddSegment`` re-keying
    ...
    try:
        ...
        for rep, seg_ids in layers.values():
            ...
            # restore names/colors by (layer, value) -- imported ids are kept,
            # not re-keyed to the source ids.
