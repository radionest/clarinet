...
- A short retrieve (`retrieve_is_complete` false — see `services/dicom/CLAUDE.md` → Short retrieves) is never cached as whole: `ensure_series_cached()` raises; `ensure_study_cached()` caches only series vouched for by `expected_counts` (C-FIND counts, passed by both service call sites), then retrieves each requested series still missing — short, or never sent — on its own via `ensure_series_cached()`, so study metadata errors instead of serving a short series. A series the PACS refuses outright (`SeriesRefusedError`) is logged and left out
