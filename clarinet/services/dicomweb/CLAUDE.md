...
- A short retrieve (`retrieve_is_complete` false — see `services/dicom/CLAUDE.md` → Short retrieves) is never cached as whole: `ensure_series_cached()` raises; `ensure_study_cached()` caches only series vouched for by `expected_counts` (C-FIND counts, passed by both service call sites) and leaves the rest out of its result
