...
- **NRRD `space` is strict**: `space directions` or `space origin` without a supported `space` (LPS/RAS/LAS) raises `ImageReadError` in `Image.read_nrrd` and `LayeredSegmentation.read_header` alike — never assume LPS in a reader. A legacy space-less file is repaired once with `declare_nrrd_space(path, "LPS")` (`image.py`; idempotent header stamp, refuses to relabel a different declared space).
