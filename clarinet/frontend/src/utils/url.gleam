...
/// Serialise a Route via `router.route_to_href` (path + query string) and
/// replace the current URL without dispatching modem's `on_url_change`.
pub fn replace_route(route: Route) -> Effect(msg) {
  use _ <- effect.from
  do_replace_state(router.route_to_href(route))
  Nil
}
