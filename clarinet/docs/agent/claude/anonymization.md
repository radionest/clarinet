...
`POST /api/dicom/studies/{uid}/anonymize?background=true` also resolves the
tracking Record by this name and returns 404 when the type does not exist.
...
async def anonymize_study_with_type(msg: PipelineMessage, ctx: TaskContext) -> None:
    ...
    study_type = first_checks[0].data.get("study_type") if first_checks else None
...
> at worker startup as soon as `have_dicom` is on. Pick a distinct name
...
`sha256(salt:study_uid)` hash instead — different across studies of the same
patient, which prevents PACS-side correlation and needs no `auto_id`.
...
| `anon_id_prefix` | `"CLARINET"` | Prefix in `anon_id`; `[A-Za-z0-9_-]+`, max 55 chars |
...
uv run clarinet anon scrub-db --patients P001,P002   # or 'all'
uv run clarinet anon scrub-db --patients all --dry-run --out dump.sql.gz
```
