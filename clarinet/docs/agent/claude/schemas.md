...
Опечатка в `source` для config-определённого RecordType валит старт: `reconcile_config` фейлится с `ConfigurationError`, называя неизвестный источник. Для типов, изменённых через API, и осиротевших записей этой проверки нет — там опечатка проявится как WARNING «Unknown x-options source» при рендере, поле останется «сырым».
