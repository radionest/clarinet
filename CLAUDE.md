...
Clarinet is an imaging-centric framework for structured research workflows, built on FastAPI, SQLModel, and async architecture.
...
uv run clarinet agent init [path]        # Install framework agent docs (.claude/rules/clarinet/) + seed .claude/CLAUDE.md
uv run clarinet agent update [path]      # Refresh managed docs, re-resolve links, prune docs no longer shipped
```
