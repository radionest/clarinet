...
**Единственная плата SSE** — лимит ~6 одновременных соединений на хост в HTTP/1.1 (WS этот пул не занимает). Снимается переходом nginx на **HTTP/2** (см. «nginx» ниже): мы уже на TLS, это одна строка конфига, и лимит исчезает как класс.

## Ключевые решения

### Авторизация: штатный `CurrentUserDep`, `authenticate_websocket` НЕ нужен

SSE-поток — это обычный HTTP GET. `EventSource` на тот же origin автоматически шлёт cookie `clarinet_session` (HttpOnly, SameSite=Lax) — её валидирует существующая зависимость `current_active_user` (`auth_config.py`, «service token → cookie fallback»), как и на любом другом роуте. Поэтому:
...
- ревалидация каждые `sse_revalidate_seconds` (default 300): внутри генератора повторный `DatabaseStrategy.read_token` по токену из `request.cookies` (свежая короткоживущая сессия БД); если сессия истекла/отозвана — сервер шлёт кадр `{"type":"auth_expired"}` и закрывает поток; клиент делает `EventSource.close()` (стоп без reconnect) + `Logout`;
...
- **Лимит 6 соединений на хост (HTTP/1.1)** — каждый SSE держит слот всю жизнь; 7-я+ вкладка виснет. Митигация — **HTTP/2** (см. nginx). Зафиксировать в docstring роутера и в nginx-комментарии.
