...
| `on_disk_dtype` | The voxel dtype declared in the file's header, independent of any load — available after any `read`/`read_nifti`/`read_nrrd` call, including `load_data=False`. NIfTI reports the header dtype directly; NRRD recognizes only uint8 and returns `None` otherwise (the one distinction `is_conform_repairable` needs). |
...
| `conform_seg_to_grid(seg_path, grid_path, *, out_path=None, atol=1e-4, allow_resample=False)` | `bool` | Repair helper: classifies the pair via `grid_relation` first — `SAME` is a no-op (`False`), `REARRANGED` (mirror/transpose) repairs by an exact index rearrangement (3-D and 4-D layered `.seg.nrrd`, layer/label-preserving) — the non-layered 3-D path also raises `ImageError` if the subject isn't already uint8 on disk (its repair reads via `Segmentation`, which forces a uint8 cast) — `FOREIGN` raises `GeometryMismatchError` unless `allow_resample=True`. For batch-fixing historically misaligned files. See [`docs/grid-workflows.md`](/docs/grid-workflows.md) for the SAME/REARRANGED/FOREIGN decision table. |
| `is_conform_repairable(seg_path)` | `bool` | Header-only probe of the dtype rule above: `True` for a 4-D layered `.seg.nrrd` (the layered path preserves the source dtype) or a 3-D file that is uint8 on disk. `conform_seg_to_grid`'s own guard and the submit-time OUTPUT policy (`grid_policy.decide`) both call it, so the policy can refuse a wider file without attempting the repair. |
...
| File | Scope | Count |
|---|---|---|
| `tests/test_image.py` | Unit tests — individual methods in isolation | 179 |
