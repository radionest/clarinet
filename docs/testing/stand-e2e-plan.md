...
| **5. Slicer real** | `worktree-headless-slicer` (foundation, PR нет, 5 коммитов off main) | 🟡 нужен PR | Stage 5b headless Slicer (порт 2016) — основа `mode=slicer` (§3.6); полная фаза зависит от 4 |
...
Три foundation-ветки независимы (разные файлы: #359 → `deploy/`, db-scrub → `clarinet/services/db_scrub`+CLI,
`headless-slicer` → `deploy/test/slicer`+test-teardown) → шли параллельно. Статус на 2026-06-15:
...
3. ⬜ **`headless-slicer`** (Фаза 5, foundation) — осталось открыть PR (5 коммитов off main); уже полезен текущему single-VM e2e (Stage 5b).

Затем по зависимостям: Фаза 1 (Multi-VM) → 3 (фикстур-пайплайн, нужны 1+2) → 4 (харнесс) → 5/6/7.
...
- **`worktree-headless-slicer` (Фаза 5, foundation).** Vendored headless Slicer (Stage 5b, порт 2016) —
  фундамент `mode=slicer` (§3.6). Уже полезен текущему single-VM e2e. Осталось открыть PR (5 коммитов off main).
