...
Статус: **активный backbone** (v2, 2026-06-15; draft v1 — 2026-06-13). Это единый трекер
...
**CLI** (реализовано — #361, в main): `clarinet anon scrub-db --patients <ids|'all'> [--out <dump.sql.gz>] [--dry-run] [--allow-phi-leak]`.
Работает **in-place** по сконфигурированной БД (оператор сначала восстанавливает прод-копию в throwaway
scratch-БД), затем опционально `pg_dump` результата в фикстуру.
...
### 5.1. Статус фаз и привязка веток (единый трекер)

Активная разработка уже идёт в нескольких ветках — это **не разные задачи, а фазы этого плана**.
Регистрируем их здесь, чтобы не делать одно и то же параллельно и не разъехаться по контрактам (§5.2).

| Фаза | Ветка / PR | Статус | Заметка |
|---|---|---|---|
| **0. Фундамент** | **PR #359** ✅ merged (`55cf7e6`) | 🟢 в main | `PROJECT_SOURCE_DIR` (деплой `plan/`+`settings.toml` downstream-проекта на VM) + settings-layering (`settings.custom.toml`) + `deploy/lib/vm-setting.sh`. Семя ролей/wiring Фазы 1 |
| **1. Multi-VM** | — | ⚪ не начато | зависит от 0 |
| **2. DB-аноним** | **PR #361** ✅ merged (`5af03d4`) | 🟢 в main | `clarinet anon scrub-db` выполняет контракт Фазы 2 (§5.2-A); ветка удалена |
| **3. Фикстур-пайплайн** | — | ⚪ не начато | зависит от 1, 2 |
| **4. Харнесс** | прототип в `docs/testing/prototype-nir_liver/` | ⚪ источник | перенос в `deploy/test/stand/` |
| **5. Slicer real** | foundation: **PR #356** ✅ merged (`2f9eabd`) | 🟢 foundation в main | Stage 5b headless Slicer (порт 2016) — основа `mode=slicer` (§3.6); полная фаза (race-ассершены + wiring `mode=slicer`) зависит от 4 |
| **6. CI** | — | ⚪ не начато | зависит от 4 |
| **7. Второй проект** | — | ⚪ не начато | зависит от 4 |

Плюс **0.10-migration-backup** (`clarinet_nir_liver/.claude/plans/`) — это single-VM **частный случай**
этого стенда (staging-VM + миграция + ручной workflow). Phase A переиспользует стенд-инфру вместо
ручной установки pg/rabbit (см. §8), Phase B остаётся операторским чек-листом.

### 5.2. Контракт-стыки между фазами (не дать веткам разъехаться)

Три точки, где ветки обязаны совпасть; фиксируем как контракт, а не как деталь реализации.

- **A. Anon path-consistency** (Фаза 2 ↔ 3 ↔ `FileRepository`). `scrub-db` ОБЯЗАН сохранять ровно то,
  чем `FileRepository.resolve_file` собирает путь к `dcm_anon/`: `patient.auto_id`
  (→ `patient.id = f"{anon_id_prefix}_{auto_id}"`, как `Patient.anon_id` и per-patient `anonymizer.py`),
  `study.anon_uid`, `series.anon_uid` (`sha256(salt:uid)→2.25.<int>`, тот же `_anon_uid`). `salt`/
  `anon_id_prefix` — те же, что в DICOM-анонимизации. Ветка `db-scrub` это уже делает (docstring
  `scrubber.py`) — закрепляем, чтобы Фаза 3 (fixture load) на это полагалась.
- **B. Settings-layering** (Фаза 0 → wiring 1/3). PR #359 ввёл `settings.custom.toml` (stand-overlay
  поверх проектного `settings.toml`, clarinet грузит с бóльшим приоритетом) и `deploy/lib/vm-setting.sh`
  (единая точка чтения эффективной настройки с VM). Per-role wiring Фаз 1/3 (`pacs_host`, `api_base_url`,
  `internal_service_token`, `rabbitmq_host`) ложится в `settings.custom.toml`, НЕ в проектный
  `settings.toml`; читать эффективную настройку — только через `vm-setting.sh`, не парсить toml руками.
- **C. Manifest как cross-phase data-contract** (Фаза 3 ↔ 4). `manifest.toml` (§3.5) — единственный
  источник demo-пациентов, spine-DAG, seg-классов, режимов стадий и race-точек. Capture (3) его пишет,
  generic-драйвер (4) читает. Прототип хардкодит это (`conftest.py`, `test_workflow.py`) → при переносе
  всё уходит в manifest.

### 5.3. Порядок выката (merge order)

Три foundation-набора независимы (разные файлы: #359 → `deploy/`, #361 → `clarinet/services/db_scrub`+CLI,
#356 → `deploy/test/slicer`+test-teardown) → **все три уже в main**. Статус на 2026-06-15:

1. ✅ **PR #359** (Фаза 0) — **смержен** (`55cf7e6`). Разблокировал Фазу 1 (роли/topology) и Phase A migration-runbook (§8).
2. ✅ **PR #361** (Фаза 2, бывш. `db-scrub`) — **смержен** (`5af03d4`); ветка удалена. Доступен Фазе 3 и анонимной копии migration-runbook.
3. ✅ **PR #356** (Фаза 5, foundation, бывш. `headless-slicer`) — **смержен** (`2f9eabd`). Headless Slicer (Stage 5b) в main; ветка squash-merged → stale, можно удалить.

Фундамент (Фазы 0/2/5) закрыт. Дальше по зависимостям: Фаза 1 (Multi-VM) → 3 (фикстур-пайплайн, нужны 1+2) → 4 (харнесс) → полная Фаза 5 (race + `mode=slicer`) / 6 / 7.

---

## 6. Открытые решения (с рекомендацией)

Зафиксировано (2026-06-13): **D1, D2 решены** (см. ниже). Обновление (2026-06-15): реализация
**активна** — Фазы 0/2/5 уже в работе в ветках (§5.1), документ ведётся как единый трекер, а не
design-doc «на потом». D3–D6 — рекомендации в силе, не противоречат; пересмотр по запросу.
...
## 8. Связь с активными ветками

Документ — единый трекер; §5.1 даёт таблицу фаз↔веток, §5.2 — контракт-стыки. Здесь контекст по каждой.

- **PR #359 — `vm-downstream-project` (Фаза 0).** ✅ Смержен (`55cf7e6`). Ввёл `PROJECT_SOURCE_DIR` (деплой
  `plan/`+`settings.toml` downstream-проекта на VM), settings-layering через `settings.custom.toml` и
  `deploy/lib/vm-setting.sh`. Правит `deploy/vm/vm.conf` (`PACS_HOST` отделён от localhost) — **семя
  разнесения PACS** из §3.1 и фундамент per-role wiring Фазы 1 (контракт §5.2-B).
- **PR #361 — `db-scrub` (Фаза 2).** ✅ Смержен (`5af03d4`), ветка удалена. `clarinet anon scrub-db` —
  главный пробел §3.2 закрыт: сужает БД до выбранных пациентов, скрабит PII во всех колонках и
  `record.data` JSON, аудит PHI. Сохраняет `auto_id`/`anon_uid` для `FileRepository` (контракт §5.2-A).
- **PR #356 — `headless-slicer` (Фаза 5, foundation).** ✅ Смержен (`2f9eabd`) — vendored headless Slicer
  (Stage 5b, порт 2016: `run-headless.sh`/`webserver.py`), фундамент `mode=slicer` (§3.6). Ветка
  squash-merged → stale (удалить). Полная Фаза 5 (race-ассершены + wiring `mode=slicer`) зависит от Фазы 4.
- **0.10-migration-backup** (`clarinet_nir_liver`, downstream). Single-VM частный случай стенда:
  staging-VM + миграция + ручной workflow. Phase A переписывается на переиспользование стенд-инфры
  (`deploy/vm` provisioning + PR #359 для деплоя ported-проекта + `scrub-db` для анонимной копии +
  stand-харнесс вместо ручного прохода по workflow); Phase B (боевой переход прода) остаётся
  операторским чек-листом. Так staging перестаёт быть hand-rolled `apt install postgres rabbitmq`.
