...
- `export_segmentation(name, output_path, *, conform_to=None)` → exports segmentation node to file. `conform_to=None` (default): exports as-is, unchanged legacy behavior. `conform_to=<reference file path>`: reads the reference's ON-DISK grid (`_read_grid_on_disk`, never a loaded node's — a loaded node may already carry Slicer's ITK-canonicalized grid) and classifies the segmentation's current grid against it via the bundled `grid_relation` (design D4): SAME exports directly; REARRANGED re-grids onto the reference by exact per-segment index rearrangement (no interpolation, `arrayFromSegmentBinaryLabelmap(..., referenceVolumeNode=...)`) onto a temporary node before exporting — the caller's node is never mutated, and segment ids, names, colors, and label values are preserved **for a source whose segments live on separate labelmap layers**. A source whose segments share one labelmap layer (e.g. loaded from a `.seg.nrrd` with custom label values) currently fails closed instead: the re-grid silently drops all voxels, and the post-write voxel-loss guard (below) raises rather than writing an empty file — known limitation, tracked as issue #500. FOREIGN raises `SlicerHelperError` without writing. Post-write, the file is re-read and re-classified against the reference grid — a mismatch deletes the file and raises (fail-closed); a second check compares source-vs-written voxel presence — a non-empty source that wrote zero voxels also deletes the file and raises (the issue #500 guard, cheap: `_segmentation_has_voxels` on the source vs. `_written_segmentation_has_voxels` on the reloaded output). Requires the correspondence bundle when `conform_to` is set — same session-sticky opt-in as `subtract_segmentations`/`detect_overlaps` (raises `SlicerHelperError` otherwise)
