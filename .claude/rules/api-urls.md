...
| `/api/records/{id}/submit` | POST | 200 | Submit + run `slicer_result_validator` if configured; merges `__execResult` into data on save. **409** when the record is `blocked`, `preparing`, or already `finished`, or when a declared OUTPUT file's grid does not conform to its reference (`on_grid_mismatch`; see `clarinet/services/grid_policy.py`). **422** when a declared INPUT file's grid no longer matches its reference (re-validated at submit time, not just at creation/check-files), and the same unsafe-OUTPUT-path rejection as POST `/data` |
| `/api/records/{id}/submit` | PATCH | 200 | Re-submit a finished record (same Slicer-validator + `__execResult` merge as POST). **409** for non-superusers when the type locks submitted records, or on the same OUTPUT grid-conformance check as POST; a successful `conform` repair re-syncs stored output checksums and fires file-change triggers |
