...
| `/api/records/{id}/validate-files` | POST | 200 | Validate files. Read-only: INPUT verdicts as at creation/check-files, plus every declared OUTPUT grid pair present on disk classified and reported as `grid_mismatch` with its `on_grid_mismatch` quoted (a preview of the submit guard); nothing is repaired or deleted |
