...
| URL | Method | Status | Description |
|---|---|---|---|
| `/api/records` | POST | 201 | Create record. **409**: `RECORD_LIMIT_REACHED`, `UNIQUE_PER_USER`, `PARENT_REQUIRED` (record_type with `parent_required=True` and no `parent_record_id` in payload). `user_id` inherited from parent only when RecordType has `inherit_user_from_parent=True` |
