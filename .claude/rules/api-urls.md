...
| `/api/records/{id}/user` | PATCH | 200 | Assign user. Auth: `AuthorizedRecordDep` (a non-superuser admin still needs the type's role), then admins assign anyone; a non-admin may only assign **themselves** to a record that is unassigned or already theirs (frontend auto-assign on open), else **403** |
...
| `/api/records/{id}/fail` | POST | 200 | Manually fail record. Auth: `AuthorizedRecordDep` + admin, else `MutableRecordDep` rules |
| `/api/records/{id}/invalidate` | POST | 200 | Invalidate record. Auth: `AuthorizedRecordDep` + admin, else `MutableRecordDep` rules. Hard mode: **409** for non-superusers when the record is finished and its type locks submitted records |
