...
- **P30.** A uniqueness/quota predicate (count/EXISTS) invoked where the candidate row is already persisted (claim/assign/update — not just create) must exclude the candidate row (`exclude_record_id`-style param) and have a test asserting the idempotent re-check case. Real bug: `ensure_unique_by` self-match at assignment → listed-but-unclaimable records.
...
- `clarinet/services/slicer/context*.py`, `plan/**/slicer_hydrators.py` → `.claude/rules/slicer-context.md`
