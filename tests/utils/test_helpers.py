...
class RecordFactory:
    ...
    async def create_record_with_relations(
        ...
        **record_kwargs: Any,
        ...
        fully-hydrated ``RecordRead`` (e.g. for ``FileRepository`` or
        ``render_slicer_args`` calls) and want to avoid the eager-load +
        DTO-validate boilerplate. Tests that need the raw ORM ``Record``
        should keep using the local ``_create_record`` helper.
        """
