...
class TestRenderItem:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (Column("at", DateTime(), server_default=func.now()), "sa.text('(CURRENT_TIMESTAMP)')"),
        ],
    )
    def test_server_default_autogenerated_on_sqlite(
        ...
        assert f"server_default={rendered}" in code
...
class TestCrossDialectRegression:
    ...
    def test_sqlite_generated_migration_applies_on_postgres(
        self, migration_project: tuple[Path, str, Engine], tmp_path: Path
    ) -> None:
        project_path, _db_url, engine = migration_project
        if engine.dialect.name != "postgresql":
            pytest.skip("cross-dialect check needs the PostgreSQL leg")
