...
class TestRenderItem:
    """``render_item`` keeps autogenerated boolean defaults dialect-neutral (#450)."""

    @staticmethod
    def _render_on_sqlite(column: Column[bool] | Column[datetime], prefix: str = "sa.") -> str:
        return render_python_code(
            ops.UpgradeOps(ops=[ops.AddColumnOp("t", column)]),
            sqlalchemy_module_prefix=prefix,
            render_item=render_item,
            migration_context=MigrationContext.configure(dialect_name="sqlite"),
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (Column("at", DateTime(), server_default=func.now()), "sa.text("),
        ],
    )
    def test_server_default_autogenerated_on_sqlite(
        self, column: Column[bool] | Column[datetime], rendered: str
    ) -> None:
        assert f"server_default={rendered}" in self._render_on_sqlite(column)

    def test_uses_configured_module_prefix(self) -> None:
        column = Column("flag", Boolean(), server_default=sql_expression.false())
        rendered = self._render_on_sqlite(column, prefix="sqlalchemy.")
        assert "server_default=sqlalchemy.false()" in rendered
