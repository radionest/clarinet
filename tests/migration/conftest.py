...
# ---------------------------------------------------------------------------
# Migration init helper (fixes sqlmodel import in autogenerated migrations)
# ---------------------------------------------------------------------------


def fix_migration_imports(project_path: Path) -> None:
    """Fix missing imports in autogenerated migration files.

    Alembic autogenerate with SQLModel produces references to:
    - sqlmodel.sql.sqltypes.AutoString (no ``import sqlmodel``)
    - Text() from sqlalchemy (no ``from sqlalchemy import Text``)
    """
...
def init_and_apply(project_path: Path) -> None:
    """Initialize Alembic, fix imports, and apply initial migration.

    init_alembic_in_project creates the migration but fails to apply it
    because autogenerate references sqlmodel types without importing sqlmodel.
    This helper patches the generated file and applies the migration.
    """
    from clarinet.utils.migrations import init_alembic_in_project, run_migrations

    init_alembic_in_project(project_path)
    fix_migration_imports(project_path)
