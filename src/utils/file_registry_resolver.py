...
class FileRegistryEntry(BaseModel):
    """A single entry in the project-level file registry.
...
async def load_project_file_registry(folder: str) -> dict[str, FileRegistryEntry] | None:
    ...
    Returns:
        Dict mapping file names to ``FileRegistryEntry`` objects, or ``None``
    ...
    if os.path.isfile(toml_path):
        ...
        registry = {name: FileRegistryEntry(**entry) for name, entry in raw.items()}
...
def resolve_file_references(
    ...
    registry: dict[str, FileRegistryEntry],
    ...
    *registry*. The registry entry provides ``pattern``, ``description``,
    ...
    Args:
        ...
        registry: Project file registry mapping names to ``FileRegistryEntry``.
...
def resolve_task_files(
    props: dict[str, Any],
    registry: dict[str, FileRegistryEntry] | None,
) -> dict[str, Any]:
