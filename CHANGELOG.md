...
- **Deactivating or deleting the admin ends `X-Internal-Token` access at once.**
  ...
  service-token superuser access until the TTL ran out. Every user mutation now
  drops that cache; `PUT` and `DELETE /api/user/{id}` also start evicting the
  target's cached sessions, as role changes and deactivation already did (#600).
