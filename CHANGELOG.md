...
- **Migrations autogenerated on SQLite now apply on PostgreSQL.** The
  generated `alembic/env.py` had no `render_item` hook, so autogenerate compiled
  the models' dialect-aware boolean `server_default`s (`sql_expression.true()` /
  `.false()`) with the database it was connected to — on SQLite, the scaffold
  default, that baked `sa.text('1')` / `sa.text('0')` into the migration, and
  PostgreSQL rejected `BOOLEAN DEFAULT 0` with `DatatypeMismatch`, in the initial
  `CREATE TABLE` as in every later `ADD COLUMN`. `clarinet init-migrations` now
  writes an `env.py` that passes `clarinet.utils.migrations.render_item`, which
  renders them as `sa.true()` / `sa.false()` — compiled by whichever database
  applies the migration (#450). **Downstream migration:** `env.py` is only
...
- **`dicom_max_concurrent_associations` now caps the worker too.** The cap
  lives in each process's memory and only the API lifespan installed it, so
  `clarinet worker` — which runs the DICOM pipeline tasks — opened associations
  without limit. Both now call `clarinet.services.dicom.install_association_cap()`
  at startup, so the worker drops from unlimited to 8 concurrent associations
  by default; raise the setting if worker retrieve throughput falls. The limit
  is per process, not fleet-wide: the API plus N workers can hold (N+1) × the
  setting at once. A value below 1 is now rejected when settings load (#551).
- **A short DICOM retrieve is no longer cached or converted as a whole one.**
  ...
  class got no presentation context (`num_failed > 0` under `warning_0xb000`) —
